...
def download_county(county: County) -> None:
    logger.info(
    ...
    if blocks.empty:
        logger.warning(
            f"No org units in GeoDataFrame for {county.name} {county.state} ({county.fips}"
        )
    else:
        ...
        logger.info(f"    Org units saved to {org_units_shapefile_name}")
